...
export class Map{
    ...
    constructor(jsonObject, gameState){
        ...
        let backgroundCompletePath = ASSETS_PATH + jsonObject.tilesets[0].image;
        let backgroundImagePath = backgroundCompletePath.split("/");
        let backgroundImageName = backgroundImagePath[backgroundImagePath.length - 1];
        let mapName = backgroundImageName.split(".");

        // if (this.gameState == listadoEstados.MAPAMUNDI) {
        //     this.mapImagePath = "img/" + nombreMapa[0] + ".mapa.png";
        // }
        // if (this.estadoJuego == listadoEstados.NIVEL) {
        //     this.mapImagePath = "img/" + nombreMapa[0] + ".nivel.png";
        // }
    
        this.tilesInWidth = parseInt(jsonObject.width);
        this.tilesInHeight = parseInt(jsonObject.height);
        this.tilesWidth = parseInt(jsonObject.tilewidth);
        this.tilesHeight = parseInt(jsonObject.tileheight);
	...
	for (let i = 0; i < layers.length; i++) {
		if (layers[i].name == "COLISIONES") {
			...
			for (let c = 0; c < layers[i].objects.length; c++) {
				this.rectangulosColisiones.push(new Rectangle(
...
Map.prototype.loadItemsOnMap = function() {
	var mapPixelsWidth = this.tilesInWidth * this.tilesWidth;
	var mapPixelsHeight = this.tilesInHeight * this.tilesHeight;
	...
	document.getElementById(idHTML).style.background = "url('" + this.mapImagePath + "')";
